...
def get_packet_loss_info(session_id):
    ...
    try:
        ...
        for link in session.links:
            ...
            link_info = {
                ...
                "loss": link.options.loss if hasattr(link, 'options') else None
            }
            links.append(link_info)

        sorted_links = sort_links(links)
        return json.dumps({"packet_loss": sorted_links}, default=str)
...
if __name__ == "__main__":
    ...
    if session_id is not None:
        print(get_packet_loss_info(session_id))
    else:
