...
class Node():
    ...
    def start(self):
        ...
        # Wait until the next 10-second mark
        current_time = datetime.datetime.now()
        seconds_to_wait = 10 - (current_time.second % 10)
    ...
    def synchronize_iterations(self, nodeServer, iteration):
        all_synchronized = False
        while not all_synchronized:
            all_synchronized = True
            for neighbor in self.neighbors:
                received_value = self.send(neighbor, iteration)
                if np.array_equal(received_value, np.array([-1])):
                    print(f"{self.HOST} waiting for {neighbor} to synchronize iteration {iteration}")
                    all_synchronized = False
                    break
    ...
    def computeSimple(self, nodeServer, nrNeighbors):
        for i in range(1, self.nrOfIterations + 1):
            # Synchronize time for iteration start
            current_time = datetime.datetime.now()
            next_iteration_time = (current_time.second // 10 + 1) * 10
            time_to_wait = next_iteration_time - current_time.second
            time.sleep(time_to_wait)

            nodeServer.ITERATION = i
            print(f"{self.HOST} is in iteration: {i}")

            # Ensure all nodes are synchronized before proceeding
            self.synchronize_iterations(nodeServer, i)
            ...
            print(f"{self.HOST} completed iteration {i}")
